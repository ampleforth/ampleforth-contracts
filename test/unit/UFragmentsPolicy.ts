...
async function mockExternalData(
  ...
  uFragSupply: BigNumberish = undefined,
  currentRateValidity = true,
  targetRateValidity = true,
) {
  await mockMarketOracle.storeData(currentRate)
  await mockMarketOracle.storeValidity(currentRateValidity)
  await mockCpiOracle.storeData(targetRate)
  await mockCpiOracle.storeValidity(targetRateValidity)
  if (uFragSupply) {
...
async function parseRebaseEvent(response: Promise<TransactionResponse>) {
  ...
  const receipt = (await r.wait()) as any
...
describe('UFragmentsPolicy', function () {
  ...
  describe('UFragmentsPolicy:initialize', async function () {
    describe('initial values set correctly', function () {
      ...
      it('should set owner', async function () {
        expect(await uFragmentsPolicy.owner()).to.eq(
  ...
  describe('UFragmentsPolicy:setOrchestrator', async function () {
    ...
    it('should set orchestrator', async function () {
      ...
      expect(await uFragmentsPolicy.orchestrator()).to.eq(
  ...
  describe('UFragments:setDeviationThreshold:accessControl', function () {
    ...
    it('should be callable by owner', async function () {
      await expect(uFragmentsPolicy.connect(deployer).setDeviationThreshold(0))
        .to.not.be.reverted
    })

    it('should NOT be callable by non-owner', async function () {
      await expect(uFragmentsPolicy.connect(user).setDeviationThreshold(0)).to
        .be.reverted
    })
  })
  ...
  describe('UFragmentsPolicy:CurveParameters', async function () {
    ...
    describe('when rebaseFunctionUpperPercentage is less than 0', async function () {
      it('should fail', async function () {
        await expect(
          uFragmentsPolicy
            .connect(deployer)
            .setRebaseFunctionUpperPercentage(-1),
        ).to.be.reverted
      })
    })
    ...
    describe('when rebaseFunctionUpperPercentage is more than 0', async function () {
      it('should setRebaseFunctionUpperPercentage', async function () {
        ...
        expect(await uFragmentsPolicy.rebaseFunctionUpperPercentage()).to.eq(
          1000,
        )
      })
    })
  ...
  describe('UFragmentsPolicy:setRebaseCircuitBreakerParameters', async function () {
    before('setup UFragmentsPolicy contract', async function () {
      ;({
        deployer,
        user,
    ...
    describe('when decline<0', function () {
      it('should fail', async function () {
        await expect(
          uFragmentsPolicy
            .connect(deployer)
            .setRebaseCircuitBreakerParameters(0, -1),
        ).to.be.reverted
      })
    })

    describe('when decline>100%', function () {
      it('should fail', async function () {
        await expect(
          uFragmentsPolicy
            .connect(deployer)
            .setRebaseCircuitBreakerParameters(
              0,
    ...
    describe('when params are valid', function () {
      it('should setRebaseTimingParameters', async function () {
        await uFragmentsPolicy
          ...
          .setRebaseCircuitBreakerParameters(
            7,
            ethers.utils.parseUnits('0.15', 18),
          )
        expect(await uFragmentsPolicy.epochLookback()).to.eq(7)
        expect(await uFragmentsPolicy.tolerableDeclinePercentage()).to.eq(
          ethers.utils.parseUnits('0.15', 18),
        )
      })
    })
  ...
  describe('UFragments:setRebaseCircuitBreakerParameters:accessControl', function () {
    before('setup UFragmentsPolicy contract', async () => {
      ;({
        deployer,
        user,
  ...
  describe('UFragmentsPolicy:Rebase', async function () {
    ...
    describe('positive rate and no change target', function () {
      ...
      it('should record supply history', async function () {
        await uFragmentsPolicy.connect(orchestrator).rebase()
        expect(await uFragmentsPolicy.supplyHistory(prevEpoch.add(1))).to.eq(
          '1065',
        )
      })
      ...
      it('should update globalAmpleforthEpochAndAMPLSupply', async function () {
        await uFragmentsPolicy.connect(orchestrator).rebase()
        await mockExternalData(INITIAL_RATE_60P_MORE, INITIAL_TARGET_RATE, 1065)
        const r = await uFragmentsPolicy.globalAmpleforthEpochAndAMPLSupply()
        expect(r[0]).to.eq(prevEpoch.add(1))
        expect(r[1]).to.eq('1065')
      })
  ...
  describe('UFragmentsPolicy:Rebase', function () {
    before('setup UFragmentsPolicy contract', async () => {
      ;({
        deployer,
        user,
      ...
      await uFragmentsPolicy.setRebaseCircuitBreakerParameters(
        7,
        ethers.utils.parseUnits('0.05', 18),
      )
      await uFragmentsPolicy.setOrchestrator(await deployer.getAddress())
    })

    describe('when uFragments shrinks and rebase attempts to go outside tolerance', function () {
      it('should limit the decline', async function () {
        await mockExternalData(INITIAL_RATE_30P_MORE, INITIAL_TARGET_RATE, 1000)
        await increaseTime(60)
        await uFragmentsPolicy.rebase()

        await mockExternalData(INITIAL_RATE_30P_MORE, INITIAL_TARGET_RATE)
        await increaseTime(60)
        await uFragmentsPolicy.rebase()

        await mockExternalData(INITIAL_RATE_2P_LESS, INITIAL_TARGET_RATE)
        await increaseTime(60)
        await uFragmentsPolicy.rebase()
